...
// window/document-during-render crashes during prerender.
//
// `main.tsx` HYDRATES this snapshot on the prerendered routes (`/`, `/en`)
// rather than re-rendering it — `hydrateRoot` adopts the painted DOM in
// place, so the prerendered hero IS the LCP element (no destroy-and-rebuild
// second paint). On any other route the served HTML is the wrong page and
// main.tsx falls back to `createRoot`. See main.tsx for the full rationale.
//
